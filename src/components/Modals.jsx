        ...
        <form onSubmit={handleFormSubmit} className="space-y-3 text-left">
          ...
          <div className="relative">
            ...
            <input
              ...
              onFocus={() => filterSuggestions(productName)}
              onBlur={() => setTimeout(() => setShowSuggestions(false), 200)}
              placeholder="Type or search product..."
              className="w-full px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-semibold text-slate-800 outline-none focus:border-sky-500"
              required
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div
                ref={listRef}
                className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-300 rounded-xl shadow-xl max-h-36 overflow-y-auto z-20"
              >
                {suggestions.map((p, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-2 text-xs font-bold border-b border-slate-100 flex justify-between cursor-pointer transition-colors ${
                        isSelected
